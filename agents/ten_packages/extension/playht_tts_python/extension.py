...
from .playht_tts import SAMPLE_RATE, PlayhtTTS, PlayhtTTSConfig
from ten import (
    AsyncTenEnv,
)
...
class Addon(AsyncTTSBaseExtension):
    ...
    async def _process_audio_data(self, ten_env: AsyncTenEnv) -> None:
        while True:
            ...
            additional_args = {"sample_rate": SAMPLE_RATE}
            await self.send_audio_out(ten_env, audio_data, **additional_args)
    ...
    async def on_request_tts(
        ...
        await self.client.text_to_speech_stream(ten_env, input_text, end_of_segment)
